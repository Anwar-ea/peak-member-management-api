...
export class ToDoService implements IToDoService {
  ...
  async getOne(
    ...
    filtersRequest: Array<IFilter<ToDo, keyof ToDo>>
  ): Promise<IToDoResponse | null> {
    return await this.todoRepository.getOneByQueryWithResponse(
      filtersRequest,
      true,
      true,
  ...
  async addMany(
    entitesRequest: IToDoRequest[],
    contextUser: ITokenUser
  ): Promise<IToDoResponse[]> {
    return (
      await this.todoRepository.addRange(
        entitesRequest.map<ToDo>((acc) => {
  ...
  async get(
    contextUser: ITokenUser,
    fetchRequest: IFetchRequest<ToDo>
  ): Promise<IDataSourceResponse<IToDoResponse>> {
    if (contextUser.privileges.includes("lawFirmRelativeDataDashboard")) {
      let userIds = await this.userRepository.find(
        { lawFirmId: new Types.ObjectId(contextUser?.lawFirmId) },
        { _id: 1 }
      );
      let lawFirmFilter = {
        field: "userId",
        values: userIds.map((x) => x._id),
      ...
      if (
        ...
        fetchRequest.queryOptionsRequest.filtersRequest.push(lawFirmFilter);
      else
        fetchRequest.queryOptionsRequest = {
          ...fetchRequest.queryOptionsRequest,
          filtersRequest: fetchRequest.queryOptionsRequest?.filtersRequest
            ? [
                ...fetchRequest.queryOptionsRequest?.filtersRequest,
                lawFirmFilter,
              ]
    ...
    return await this.todoRepository.getPagedData(
      fetchRequest,
      true,
      true,
      contextUser.accountId
  ...
  async partialUpdate(
    id: string,
    partialEntity: Partial<IToDoRequest>,
    contextUser: ITokenUser
  ): Promise<IToDoResponse> {
  ...
  async updatePriority(
    ...
    const bulkOps = reorderedTodos
      .reverse()
      .map<AnyBulkWriteOperation<ToDo>>((todo, index) => ({
        updateOne: {
          filter: { _id: todo._id },
          update: {
            priority: index,
  ...
  async updateMany(
    entitesRequest: (IToDoRequest & { id: string })[],
    contextUser: ITokenUser
  ): Promise<any> {
    return await this.todoRepository.updateRange(
      entitesRequest.map<ToDo>((acc) => {
  ...
  async softDelete(id: string, contextUser: ITokenUser): Promise<void> {
  ...
  async deleteMany(ids: string[], contextUser: ITokenUser): Promise<void> {
    let todos = await this.todoRepository.deleteRange({
      _id: { $in: ids.map((id) => new Types.ObjectId(id)) },
    });

    if (todos.length !== ids.length)
      throw new Error(`Some todo with provided ids not found`);
  }
}
