...
export class UserService implements IUserService {
  constructor(
    ...
    private readonly privilegeRepository: IPrivilegeRepository,
    @inject("MeasurableRepository")
    private readonly measureableRepoisitory: IMeasurableRepository
  ) {}
  ...
  public async generateLoginToken(
    ...
    const chain = this.sites.reduceRight((nextRedirect, site) => {
      const url = `${site}/sso-ep/?sso_token=${encodeURIComponent(
        token
      )}&redirect=${encodeURIComponent(nextRedirect)}`;
      return url;
  ...
  async login(
    ...
    const redirectUrl = `${process.env.APP_FRONTEND_URL}/dashboard`; // Set your desired redirect URL after login
  ...
  async loginBySSO(
    sso_token: string
  ): Promise<(IUserResponse & { token: string }) | undefined> {
    if (!sso_token) {
  ...
  async loginAsMember(
    ...
    user.lastLogin = new Date();
    await this.userRepository.update(user._id.toString(), user);
    let privilages = await this.privilegeRepository.find({
      _id: { $in: user.role?.privilegeIds ?? [] },
    });
    ...
    return {
      ...
      token: signJwt({
        ...
        privileges: user.role?.privileges
          ? user.role.privileges.map((x) => x.code)
          : [],
      }),
    };
  }
  ...
  async getOne(
    ...
    filtersRequest: Array<IFilter<User, keyof User>>
  ): Promise<IUserResponse | null> {
    return await this.userRepository.getOneByQueryWithResponse(
      filtersRequest,
      true,
      true,
  ...
  async addMany(
    entitesRequest: IUserRequest[],
    contextUser: ITokenUser
  ): Promise<IUserResponse[]> {
    return (
      await this.userRepository.addRange(
        entitesRequest.map<User>((acc) => {
  ...
  async get(
    contextUser: ITokenUser,
    fetchRequest: IFetchRequest<User>
  ): Promise<IDataSourceResponse<IUserResponse>> {
    return await this.userRepository.getPagedData(
      fetchRequest,
      true,
      true,
      contextUser.accountId
  ...
  async partialUpdate(
    id: string,
    entityRequest: Partial<IUserRequest>,
    contextUser: ITokenUser
  ): Promise<IUserResponse> {
  ...
  async resetPassword(
    ...
    let updatedEntity = this.userRepository.update(
  ...
  async updateMany(
    entitesRequest: (IUserRequest & { id: string })[],
    contextUser: ITokenUser
  ): Promise<any> {
    return this.userRepository.updateRange(
      entitesRequest.map<User>((acc) => {
  ...
  async deleteMany(ids: string[], contextUser: ITokenUser): Promise<void> {
    let users = await this.userRepository.deleteRange({
      _id: { $in: ids.map((id) => new Types.ObjectId(id)) },
    });

    if (users.length !== ids.length)
      throw new Error(`Some user with provided ids not found`);
  }

  async dropdown(accountId: string): Promise<IDropdownResponse[]> {
    return await this.userRepository.dropdown(accountId, [
      "firstName",
      "lastName",
    ]);
  }
