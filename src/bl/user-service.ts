...
export class UserService implements IUserService {
    ...
    async login(loginRequest: ILoginRequest): Promise<IUserResponse & {token: string}> {
        ...
        if (match) {
            ...
            return {...user.toResponse(user), token: signJwt({id: user._id.toString(), name: `${user.firstName} ${user.lastName}`, accountId: user.accountId.toString(), privileges: user.role?.privileges ? user.role.privileges.map(x => x.code) : []})};
        }
        else {
