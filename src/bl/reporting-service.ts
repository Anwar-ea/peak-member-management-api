...
import {
  ICustomMeasurableValueResponse,
  IMeasurableResponse,
  IReportTotals,
  ITokenUser,
} from "../models";
...
export class ReportingService implements IReportingService {
  ...
  async get(
    ...
    userId?: string
  ): Promise<Array<IMeasurableReport>> {
    ...
    let measurableTypes: Array<GoalUnits> = [GoalUnits.Custom, GoalUnits.Debt];
    ...
    if (contextUser.privileges.includes("seeRevenueScoreCard"))
      measurableTypes.push(GoalUnits.Revenue);
    if (contextUser.privileges.includes("seeRetentionScoreCard"))
      measurableTypes.push(GoalUnits.RetentionRate);

    let result = await this.userRepository.aggregate<
      User & {
        measurables: Array<
          Measurable & { customValues: Array<CustomMeasurableValue> }
        >;
    ...
    for (let res of result) {
      ...
      for (let measurable of res.measurables) {
        let avgCumObj: IMeasurableResponse & {
          average?: number;
          cumulative?: number;
          customValues: Array<ICustomMeasurableValueResponse>;
        } = { ...new Measurable().toResponse(measurable), customValues: [] };

        if (measurable.showAverage && measurable.averageStartDate) {
          let averageStartDateWeekNumber = moment(
            measurable.averageStartDate
          ).week();
          switch (measurable.unit) {
            case GoalUnits.Revenue:
              let revenueToShow = res.revenues.filter(
                (x) =>
                  x.week >= averageStartDateWeekNumber &&
                  x.week <= currentWeekNumber
              );
              avgCumObj.average =
                revenueToShow.reduce(
                  (accumulator, currentValue) =>
                    accumulator + currentValue.revenue,
                  0
                ) / revenueToShow.length;
              break;
            case GoalUnits.RetentionRate:
              let retentionToShow = res.retentions.filter(
                (x) =>
                  x.week >= averageStartDateWeekNumber &&
                  x.week <= currentWeekNumber
              );
              avgCumObj.average =
                retentionToShow.reduce(
                  (accumulator, currentValue) =>
                    accumulator +
                    parseFloat(
                      (
                        currentValue.retained /
                        (currentValue.appointments
                          ? currentValue.appointments / 100
                          : 1)
                      ).toFixed(2)
                    ),
                  0
                ) / retentionToShow.length;
              avgCumObj.average = parseFloat(avgCumObj.average.toFixed(2));
              break;
            case GoalUnits.Custom:
              let customToShow = measurable.customValues.filter(
                (x) =>
                  x.week >= averageStartDateWeekNumber &&
                  x.week <= currentWeekNumber
              );
              ...
              break;
            case GoalUnits.Debt:
              let debtToShow = measurable.customValues.filter(
                (x) =>
                  x.week >= averageStartDateWeekNumber &&
                  x.week <= currentWeekNumber
              );
        ...
        if (measurable.showCumulative && measurable.cumulativeStartDate) {
          let cumulativeStartDateWeekNumber = moment(
            measurable.cumulativeStartDate
          ).week();
          switch (measurable.unit) {
            case GoalUnits.Revenue:
              let revenueToShow = res.revenues.filter(
                (x) =>
                  x.week >= cumulativeStartDateWeekNumber &&
                  x.week <= currentWeekNumber
              );
              avgCumObj.cumulative = revenueToShow.reduce(
                (accumulator, currentValue) =>
                  accumulator + currentValue.revenue,
                0
              );
            ...
            case GoalUnits.RetentionRate:
              ...
              avgCumObj.cumulative = retentionToShow.reduce(
                (accumulator, currentValue) =>
                  accumulator + currentValue.retained,
                0
              );
              break;
            case GoalUnits.Custom:
            case GoalUnits.Debt:
              ...
              avgCumObj.cumulative = customToShow.reduce(
        ...
        avgCumObj.customValues = measurable.customValues.map((x) =>
          new CustomMeasurableValue().toResponse(x)
        );
        responseObject.measurables.push(avgCumObj);
      }
      responseObject.revenues = res.revenues.map((x) =>
        new Revenue().toResponse(x)
      );
      responseObject.retentions = res.retentions.map((x) =>
        new Retention().toResponse(x)
      );
  ...
  async reportTotals(accountId: string): Promise<IReportTotals> {
    let res = await this.accountRepository.aggregate<IReportTotals>(
      reportTotalsQuery(accountId, moment().get("year"))
    );
    return res.length > 0
      ? res[0]
      : {
